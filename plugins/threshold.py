...
thresholdScorer2= {
    ...
    "children":[
        ...
        {"name":"initialized","type":"variable","value":False}, # the initialization: if set False, the next time this function is called, we reset all things and create all variables and links fresh
                                                                # which is effectively initializing the function, on the second and following calles, it does its normal job
...
def threshold_scorer_2(functionNode):
    ...
    """
    logger = functionNode.get_logger()
    logger.debug("threshold_scorer_2()")
    if not functionNode.get_child("initialized").get_value():
        #this is the first call, we just initialize
        threshold_scorer_2_init(functionNode)
        functionNode.get_child("initialized").set_value(True)
        return
